...
#sss = StratifiedShuffleSplit(n_splits=200, test_size=0.1, random_state=0)
#predictions = [[],[]]
#null_predictions = [[], []]
#scores = []
#null_scores = []
...
#    random_y = np.random.permutation(len(train_index))
#    y_shuffle = y[train_index[random_y]]
#    clf.fit(X[train_index], y_shuffle)
#    outputs_shuffle = clf.predict(X[test_index])
#    null_scores.append(roc_auc_score(y[test_index], outputs_shuffle, average='weighted', sample_weight=None))
#    #null_predictions[0].extend(y[test_index])
#    #null_predictions[1].extend(outputs_shuffle)
...
for idx in range(Y.shape[1]):
    ...
    #for train, test in ShuffleSplit(n_splits=200, random_state=0, test_size=0.2, train_size=None):   
        clf1 = ExtraTreesRegressor(n_estimators=2000, max_depth=None, min_samples_split=2)
...
#        outputs = clf.predict(X[test])
#        
#        clf.fit(X[train], Y[train, idx]).predict(X[test])
        outputs = clf.fit(X[train], Y[train,  idx]).predict(X[test])
        
        indices.extend(test)
        #scores.append(roc_auc_score(Y[test, idx], outputs, average='weighted', sample_weight=None))
        #scores.append(auc(Y[test, idx], outputs))
        scores=np.append(scores,(explained_variance_score(Y[test, idx], outputs)))
        #scores.append(explained_variance_score(Y[test, idx], outputs))
#        importances.append(clf.steps[-2][1].inverse_transform(clf.steps[-1][1].feature_importances_[None, :]))
        importances=np.append(importances,clf.steps[-2][1].inverse_transform(clf.steps[-1][1].feature_importances_[None, :]))
        predictions[0].extend(Y[test, idx])
        predictions[1].extend(outputs)
        random_y = np.random.permutation(len(train))
        #random_y = np.random.permutation(len(X))
        y_shuffle = Y[train[random_y]]
#        clf.fit(X[train], y_shuffle)
#        outputs_shuffle = clf.predict(X[test])
        outputs_shuffle = clf.fit(X[train], y_shuffle[:,idx]).predict(X[test])
        #null_scores.append(roc_auc_score(Y[test], outputs_shuffle, average='weighted', sample_weight=None))
        
        
        #null_scores.append(explained_variance_score(Y[test, idx], outputs_shuffle))
        null_scores=np.append(null_scores,(explained_variance_score(Y[test, idx], outputs_shuffle)))
...
#    score = explained_variance_score(results[0], results[1])
    allresults[lsas_keys[idx]] = {'results': results, 'r2': scores,
...
wilcoxon(scores, null_scores, correction=True)
#
#
#
#
